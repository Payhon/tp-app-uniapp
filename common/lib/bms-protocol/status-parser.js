import { RegisterView, decodeAscii } from './register-view.js';

function decodeTempCFromOffset(rawByte, offset = -40) {
	if (rawByte === 0xff) return null;
	return rawByte + offset;
}

function decodeBmsCellTempCFromKelvin10(rawU16) {
...
function allSame(bytes, v) {
	for (let i = 0; i < bytes.length; i += 1) if (bytes[i] !== v) return false;
	return true;
}

function decodeBitField32(u32, mapping) {
	const out = {};
