 ...
 * @property {String}			mode				展示格式 mode=date为日期选择，mode=time为时间选择，mode=year为年选择，mode=year-month为年月选择，mode=datetime为日期时间选择  ( 默认 ‘datetime )
...
export default {
	...
	methods: {
		...
		confirm() {
			if(this.showInput){
				this.inputValue = this.getFormatValue(this.innerValue, this.format)
			}

			let innerValue = this.valueFormat ? this.getFormatValue(this.innerValue, this.valueFormat) : this.innerValue
		...
		correctValue(value) {
			...
			if(this.showInput && value){
				this.inputValue = this.getFormatValue(value, this.format)
			}
			...
			if (isDateMode && (!value || !dayjs(value).isValid())) {
				...
				const now = Date.now()
				// 检查当前时间是否在最大最小时间范围内
				if (now >= this.minDate && now <= this.maxDate) {
					// 如果当前时间在有效范围内，使用当前时间
					value = now
				} else {
					// 如果当前时间不在范围内，使用最小时间为当前时间
					value = this.minDate
				}
				...
				// 检查当前时间是否在最小最大小时分钟范围内
				if (currentHour >= this.minHour && currentHour <= this.maxHour &&
					currentMinute >= this.minMinute && currentMinute <= this.maxMinute) {
					// 如果当前时间在有效范围内，使用当前时间
					value = `${uni.$u.padZero(currentHour)}:${uni.$u.padZero(currentMinute)}`
				} else {
					// 如果当前时间不在范围内，使用最小时间
