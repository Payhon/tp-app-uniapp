		...
		<view class="tp-box tp-box-sizing tp-flex tp-flex-col">
			...
			<uni-popup ref="addFormPopup" type="bottom" :mask="true" :maskClick="true">
				<view class="logInfo">
					...
					<view class="info_btn">
						<view class="btn_del" @click="closeAddFormPopup">{{ $t('common.cancel') }}</view>
...
type WarningAddRes = ApiResponse<unknown>
...
onShow(() => {
	uni.setNavigationBarTitle({ title: t('pages.addAlertStrategy') as string })
})
...
const doUpdateSubmit = async () => {
	...
	try {
		const res = (await req<unknown>('/api/warning/add', params, 'post')) as WarningAddRes
		showMsg(res.message || '')
		if (res.code === 200) {
			uni.navigateBack(-1)
		}
...
const toSelectTj = async (rule: RuleItem) => {
	const req = apiRequest
	if (!req) return
