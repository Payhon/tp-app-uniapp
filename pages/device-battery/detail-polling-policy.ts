...
export const DETAIL_MQTT_PARAMS_KEEPALIVE_INTERVAL_MS = 10_000
export const DETAIL_MQTT_HISTORY_KEEPALIVE_INTERVAL_MS = 30_000
...
export function resolveDetailPollingPolicy(options: {
	...
	if (options.tab === 2) {
		...
		return {
			mode: 'mqtt_keepalive',
			intervalMs: DETAIL_MQTT_PARAMS_KEEPALIVE_INTERVAL_MS,
			initialDelayMs: DETAIL_MQTT_PARAMS_KEEPALIVE_INTERVAL_MS,
		}
	}
	...
	if (options.tab === 3) {
		...
		return {
			mode: 'mqtt_keepalive',
			intervalMs: DETAIL_MQTT_HISTORY_KEEPALIVE_INTERVAL_MS,
			initialDelayMs: DETAIL_MQTT_HISTORY_KEEPALIVE_INTERVAL_MS,
		}
	}
