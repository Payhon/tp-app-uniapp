	...
	<view class="setting-page">
		<view class="card">
			...
			<view class="row" hover-class="row--hover" @tap="handleEditNickname">
				...
				<view class="right">
					<text class="value" :class="{ 'value--placeholder': !nicknameDisplay }">{{ nicknameDisplay || $t('pages.my.settingPage.nicknameUnset') }}</text>
					<u-icon name="arrow-right" size="16" color="#C0C4CC"></u-icon>
				</view>
			</view>
		</view>
		...
		<view class="card card--mt">
			<view class="row" :hover-class="hasUsername ? '' : 'row--hover'" @tap="goSetUsername">
				<text class="label">{{ $t('pages.my.settingPage.username') }}</text>
				<view class="right">
					<text class="value" :class="{ 'value--placeholder': !hasUsername }">
						{{ hasUsername ? username : $t('pages.my.settingPage.setUsername') }}
					</text>
					<u-icon v-if="!hasUsername" name="arrow-right" size="16" color="#C0C4CC"></u-icon>
				</view>
			</view>
		</view>
		<!-- #endif -->
		...
		<view class="card card--mt">
			<view class="row-head">
				<text class="head-title">{{ $t('pages.my.settingPage.accountBinding') }}</text>
			</view>

			<!-- 手机号绑定 -->
			<!-- #ifdef MP-WEIXIN -->
			<view v-if="phoneBound" class="row" hover-class="row--hover" @tap="toastAlreadyBound">
			...
			>
				<view class="row row--btn">
					<text class="label">{{ $t('pages.my.settingPage.bindPhone') }}</text>
					<view class="right">
						<text class="value value--placeholder">{{ $t('pages.my.settingPage.unbound') }}</text>
						<u-icon name="arrow-right" size="16" color="#C0C4CC"></u-icon>
					</view>
				</view>
			</button>
			...
			<view class="row" hover-class="row--hover" @tap="openBindPopup('phone')">
				...
				<view class="right">
					...
					<u-icon v-if="!phoneBound" name="arrow-right" size="16" color="#C0C4CC"></u-icon>
				</view>
			</view>
			<!-- #endif -->
			...
			<view class="row" hover-class="row--hover" @tap="openBindPopup('email')">
				...
				<view class="right">
					...
					<u-icon v-if="!emailBound" name="arrow-right" size="16" color="#C0C4CC"></u-icon>
				</view>
			</view>
			...
			<view class="row" hover-class="row--hover" @tap="bindWeChat">
				...
				<view class="right">
					<text class="value" :class="{ 'value--placeholder': !wechatBound }">
						{{ wechatBound ? $t('pages.my.settingPage.bound') : $t('pages.my.settingPage.unbound') }}
					</text>
		...
		<view class="logout-wrap">
			<u-button :text="$t('pages.accounts.logout')" shape="circle" type="primary" color="#0B3BFF" @click="doLogout"></u-button>
		</view>

		<!-- #ifdef MP-WEIXIN -->
		<u-wx-auth :show="wxAuthVisible" @close="wxAuthVisible = false" @confirm="onWxAuthConfirm"></u-wx-auth>
		<!-- #endif -->

		<u-popup :show="bindPopupVisible" mode="bottom" :round="16" @close="bindPopupVisible = false">
			<view class="popup">
				...
				<view class="popup-form">
					<template v-if="bindType === 'phone'">
						<view class="field">
							<text class="field-label">{{ $t('pages.my.settingPage.phone') }}</text>
							<input class="field-input" type="text" v-model="bindPhone" :placeholder="$t('pages.my.settingPage.phonePlaceholder')" />
						</view>
					</template>

					<template v-else>
						<view class="field">
							<text class="field-label">{{ $t('pages.my.settingPage.email') }}</text>
							<input class="field-input" type="text" v-model="bindEmail" :placeholder="$t('pages.my.settingPage.emailPlaceholder')" />
						</view>
					</template>

					<view class="field field--code">
						<text class="field-label">{{ $t('pages.my.settingPage.verifyCode') }}</text>
						<view class="code-row">
							<input class="field-input field-input--code" type="text" v-model="bindCode" :placeholder="$t('pages.my.settingPage.codePlaceholder')" />
							<u-button
		...
		<u-popup :show="nicknamePopupVisible" mode="bottom" :round="16" @close="nicknamePopupVisible = false">
			<view class="popup">
				<view class="popup-title">{{ $t('pages.my.settingPage.editNickname') }}</view>
				<view class="popup-form">
					<view class="field">
						<text class="field-label">{{ $t('pages.my.settingPage.nickname') }}</text>
						<input class="field-input" type="text" v-model="nicknameInput" :placeholder="$t('pages.my.settingPage.nicknamePlaceholder')" />
					</view>
				</view>
				<view class="popup-actions">
					<view class="popup-actions__cancel">
...
type BindingItem = { identity_type: string; identifier: string; status?: string }
...
const phoneBinding = computed(() => bindings.value.find((x) => x.identity_type === 'PHONE'))
const emailBinding = computed(() => bindings.value.find((x) => x.identity_type === 'EMAIL'))
const wechatBinding = computed(() => bindings.value.find((x) => x.identity_type === 'WXMP_OPENID'))
...
const displayName = computed(() => String(userStore.userInfo?.name || '').trim() || (t('pages.my.defaultName') as string))
...
const wxNickName = computed(() => {
	...
	const additional = parseAdditionalInfo(u?.additional_info ?? u?.additionalInfo)
...
const username = computed(() => String(userStore.userInfo?.name || '').trim())
...
const avatarSrc = computed(() => {
	...
	const baseUrl = String(getBaseUrl() || '').replace(/\/?api\/v1\/?$/, '').replace(/\/$/, '')
...
const maskedPhone = computed(() => maskPhone(phoneBinding.value?.identifier || ''))
...
const maskedEmail = computed(() => maskEmail(emailBinding.value?.identifier || ''))

const toastAlreadyBound = () => {
	uni.showToast({ title: t('pages.my.settingPage.alreadyBound') as string, icon: 'none' })
}
...
const loadBindings = async () => {
	...
	try {
		const res = await apiRequest<BindingsResp>('/api/v1/app/auth/bindings', {}, 'get')
		if (res && (res as any).code == 200) bindings.value = (((res as any).data?.list || []) as BindingItem[]) || []
		else bindings.value = []
	} finally {
...
const refreshUserInfo = async () => {
	...
	try {
		...
		if (res && (res as any).code == 200 && (res as any).data) userStore.setUserInfo((res as any).data as any)
	} catch {}
}
...
const handleEditAvatar = async () => {
	...
	try {
		...
		const res = await apiRequest<unknown>('/api/v1/app/auth/profile', { avatar_url: uploadedPath }, 'post')
		if (res && (res as any).code == 200) {
			userStore.editUserInfoField({ key: 'avatar_url', value: uploadedPath })
			await refreshUserInfo()
			uni.showToast({ title: t('pages.my.settingPage.updateSuccess') as string, icon: 'none' })
		} else {
			uni.showToast({ title: (res as any)?.message || (t('pages.my.settingPage.updateFailed') as string), icon: 'none' })
		}
	} catch {
		// ignore
	} finally {
...
const saveNickname = async () => {
	...
	if (!nick) {
		uni.showToast({ title: t('pages.my.settingPage.nicknamePlaceholder') as string, icon: 'none' })
		return
	}
	savingNickname.value = true
	try {
		const res = await apiRequest<unknown>('/api/v1/app/auth/profile', { nick_name: nick }, 'post')
		if (res && (res as any).code == 200) {
...
// #ifdef MP-WEIXIN
const onWxAuthConfirm = async (payload: { avatar?: string; nickname?: string }) => {
	...
	try {
		...
		const res = await apiRequest<unknown>('/api/v1/app/auth/wxmp/profile', { nick_name: nickname || undefined, avatar_url: uploadedPath || undefined }, 'post')
		if (res && (res as any).code == 200) {
			if (uploadedPath) userStore.editUserInfoField({ key: 'avatar_url', value: uploadedPath })
			await refreshUserInfo()
			uni.showToast({ title: t('pages.my.settingPage.updateSuccess') as string, icon: 'none' })
		} else {
			uni.showToast({ title: (res as any)?.message || (t('pages.my.settingPage.updateFailed') as string), icon: 'none' })
		...
		uni.showToast({ title: t('pages.my.settingPage.updateFailed') as string, icon: 'none' })
	} finally {
		uni.hideLoading()
	}
}
...
const onWxGetPhoneNumber = async (e: { detail?: { code?: string } }) => {
	...
	if (!code) {
		uni.showToast({ title: t('pages.my.settingPage.wxPhoneDenied') as string, icon: 'none' })
		return
	}
	if (!apiRequest) return
	uni.showLoading({ title: t('common.loading') as string })
	try {
		const res = await apiRequest<unknown>('/api/v1/app/auth/wxmp/bind_phone', { phone_code: code }, 'post')
		if (res && (res as any).code == 200) {
			uni.showToast({ title: t('pages.my.settingPage.bindSuccess') as string, icon: 'none' })
			loadBindings()
			refreshUserInfo()
		} else {
			uni.showToast({ title: (res as any)?.message || (t('pages.my.settingPage.bindFailed') as string), icon: 'none' })
		}
	} finally {
		uni.hideLoading()
	}
...
const bindWeChat = async () => {
	...
	try {
		...
		const res = await apiRequest<unknown>('/api/v1/app/auth/wxmp/bind', { code }, 'post')
		if (res && (res as any).code == 200) {
			uni.showToast({ title: t('pages.my.settingPage.bindSuccess') as string, icon: 'none' })
			loadBindings()
		} else {
		...
		uni.showToast({ title: t('pages.my.settingPage.bindFailed') as string, icon: 'none' })
	} finally {
		uni.hideLoading()
	}
	// #endif
	// #ifndef MP-WEIXIN
	uni.showToast({ title: t('pages.my.settingPage.wechatBindOnlyWxmp') as string, icon: 'none' })
	// #endif
}
...
const bindPopupTitle = computed(() =>
	bindType.value === 'phone' ? (t('pages.my.settingPage.bindPhone') as string) : (t('pages.my.settingPage.bindEmail') as string)
)
...
const sendBindCode = async () => {
	...
	try {
		...
		if (bindType.value === 'phone') {
			const pn = String(bindPhone.value || '').trim().replace(/\s+/g, '')
			if (!pn) {
				uni.showToast({ title: t('pages.my.settingPage.phonePlaceholder') as string, icon: 'none' })
				return
			}
			const res = await apiRequest<unknown>('/api/v1/app/auth/phone/code', { phone_prefix: '+86', phone_number: pn, scene }, 'post')
			if (res && (res as any).code == 200) {
				startCountdown()
				uni.showToast({ title: t('pages.my.settingPage.codeSent') as string, icon: 'none' })
			} else {
				uni.showToast({ title: (res as any)?.message || (t('pages.my.settingPage.sendCodeFailed') as string), icon: 'none' })
			}
			return
		}
		...
		if (!em) {
			uni.showToast({ title: t('pages.my.settingPage.emailPlaceholder') as string, icon: 'none' })
			return
		}
		const res = await apiRequest<unknown>('/api/v1/app/auth/email/code', { email: em, scene }, 'post')
		if (res && (res as any).code == 200) {
			startCountdown()
			uni.showToast({ title: t('pages.my.settingPage.codeSent') as string, icon: 'none' })
		} else {
...
const submitBind = async () => {
	...
	if (!code) {
		uni.showToast({ title: t('pages.my.settingPage.codePlaceholder') as string, icon: 'none' })
		return
	}
	...
	try {
		if (bindType.value === 'phone') {
			...
			if (res && (res as any).code == 200) {
				userStore.editUserInfoField({ key: 'mobile', value: pn })
		...
		const res = await apiRequest<unknown>('/api/v1/app/auth/bind/email', { email: em, verify_code: code }, 'post')
		if (res && (res as any).code == 200) {
